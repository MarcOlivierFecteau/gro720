...
def assert_almost_equal(a: Array1D | float, b: Array1D | float) -> None:
...
if __name__ == "__main__":
    ...
    if mode == "test":
        ...
        x_train = np.genfromtxt(path.join(root, "train.csv"), delimiter=",")[:, slice(0, 2)]
        target_train = np.expand_dims(
            np.genfromtxt(path.join(root, "train.csv"), delimiter=",")[:, 2], axis=1
        )
        x_val = np.genfromtxt(path.join(root, "val.csv"), delimiter=",")[:, slice(0, 2)]
        target_val = np.expand_dims(
            np.genfromtxt(path.join(root, "val.csv"), delimiter=",")[:, 2], axis=1
        )
        train(
            x_train,
            target_train,
