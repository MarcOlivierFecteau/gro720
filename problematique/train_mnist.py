...
def main():
    ...
    parser.add_argument(
        "--learning_rate", type=float, help="Choose the learning rate", required=True
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    trainer = MnistTrainer(
        network, args.learning_rate, args.epoch_count, args.batch_size, args.output_path
    )
    trainer.train()
...
def create_network(checkpoint_path):
    layers = [
        ...
        BatchNormalization(32, alpha=0.1),
        ReLU(),
